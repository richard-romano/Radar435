...
#filter
signal_envelope_filter=butter_bandpass_filter(signal_envelope, 3800,6200,fs,order=5)
...
correlation=abs(scipy.signal.correlate(signal_envelope_filter,kernel,'full'))
...
n=5
su_Prob=0.9
Noise=statistics.median([correlation])
...
peaks, _ = find_peaks(correlation, height=su_Prob*S_max,distance=2e3)

nsamps = len(correlation)
x = np.linspace(0,R_unamb, nsamps)
dx = R_unamb/(len(correlation))


plt.plot(x/1e3, correlation)
plt.plot(peaks*dx/1e3,correlation[peaks], 'x')
